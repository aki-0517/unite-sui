...
module fusion_plus_crosschain::time_lock {
    use sui::clock::{Clock};
    ...
    /// Creates a time lock for a specific duration from now
    /// @param duration Duration in milliseconds
    /// @param clock The clock reference
    /// @return u64 The time lock timestamp
    public fun create_time_lock(duration: u64, clock: &Clock): u64 {
    ...
    public fun test_time_lock_functionality(clock: &Clock) {
        ...
        let time_lock = create_time_lock(duration, clock);
